...
Agents are specialized subprocesses that tstack commands spawn to do focused work. Each agent has a fixed model tier, a scope lock, and a defined return contract so the calling command knows exactly what it gets back.
...
See [extensions.md](extensions.md) for priority resolution and [install.md](install.md) for how project templates bundle agents.
